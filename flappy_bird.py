...
pg.init() #initalise to run pygame
#we made the game all the data of game will be in it 
class Game:
    def __init__(self):  #constructor is init
        #setting window configration
        self.width =600
        self.height=768
        self.scale_factor=1.5
        
        self.win=pg.display.set_mode((self.width,self.height)) #we make the winodow and we have to give a tuple inside
        self.clock=pg.time.Clock()

        self.move_speed=250
        self.start_monitoring=False
        self.score=0
        self.font=pg.font.Font("sfx/font.ttf",24)
    ...
    def gameLoop(self):
        last_time=time.time()
        while True: #game loop to continue the game 
...
#draw the pictures we just load them
    def drawEverything(self):
            
            self.win.blit(self.bg_img,(0,-300)) #to show image text sprits in pygame use blit
            for pipe in self.pipes:
    ...
    def setUpBgAndGround(self):
        ...
        self.bg_img=pg.transform.scale_by(pg.image.load("sfx/bg.png").convert(),self.scale_factor)
        self.ground1_img=pg.transform.scale_by(pg.image.load("sfx/ground.png").convert(),self.scale_factor)
        self.ground2_img=pg.transform.scale_by(pg.image.load("sfx/ground.png").convert(),self.scale_factor)
